...
glm1 <- glm(Tr ~ age + citizenship + gender + ethnicities + hs_rank_2 + family_income + cola_abbr1, family=binomial, data = cola_before)
...
```{r before_after_match, message=FALSE, warning=FALSE, include=FALSE}
 ...
 MatchBalance(Tr ~ age + citizenship + gender + ethnicities + hs_rank_2 + family_income + cola_abbr1, match.out = rr1, nboots = 1000, data = cola_before)
...
#matchbalance MatchBalance(Tr ~ age + citizenship + gender + ethnicities + hs_rank_2 + family_income + cola_abbr1 , match.out = rr1, nboots = 1000, data = cola_before)
...
reg_df <- after %>% left_join(before, by = "eid") %>% dplyr::select(eid, gpa_before, gpa_after, treatment, treatment_strength, year)
...
reg_dummies <-
  lm(gpa_after ~ gpa_before + treatment + treatment_strength + factor(year),
     data = reg_df)
summary(reg_dummies)
